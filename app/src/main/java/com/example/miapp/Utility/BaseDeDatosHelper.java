...
public class BaseDeDatosHelper extends SQLiteOpenHelper {
    ...
    public int insertarPublicacion(PublicacionCabecera publicacionCabecera) {
        long dato = 0;
        try {
            ...
            dato=db.insert(TABLA_PUBLICACIONES_CABECERA, null, valores);
            db.close();
        } catch (Exception ex) {
            Log.d("INSERTERROR", "la concha:" + ex.toString());
            throw ex;
        }
        return (int) dato;
    }
    public int insertarPublicacionDetalle(int id_publicacion,int id_empresa, List<String> imagenes_list) {
        long dato = 0;
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues valores = new ContentValues();
            for (int i = 0; i < imagenes_list.size(); i++) {
                ...
                valores.put("imagen_documento", imagenes_list.get(i).toString());
                dato=db.insert(TABLA_PUBLICACIONES_DETALLE, null, valores);
            }
            db.close();
        } catch (Exception ex) {
            Log.d("INSERTERROR", "la concha:" + ex.toString());
    ...
    public List<PublicacionCabecera> obtenerPublicacionCabecera(int id_empresa_) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLA_PUBLICACIONES_CABECERA+" where id_empresa="+id_empresa_, null);
    ...
    public List<PublicacionCabecera.PublicacionDetalle> obtenerPublicacionDetalle(int id_empresa_) {
        List<PublicacionCabecera.PublicacionDetalle> lista = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLA_PUBLICACIONES_DETALLE+" where id_empresa="+id_empresa_, null);
        if (cursor.moveToFirst()) {
            do {
                int id_publicacion = cursor.getInt(cursor.getColumnIndexOrThrow("id_publicacion"));
                ...
                lista.add(new PublicacionCabecera.PublicacionDetalle(id_publicacion,id_empresa,secuencial,imagen_documento));
            } while (cursor.moveToNext());
        }
        cursor.close();
        db.close();
